...
export default function Features() {
  const highlights = [
    ...
    {
      icon: <ClipboardCheck className="h-6 w-6 text-amber-400" />,
    ...
    <section id="why-us" className="py-24 bg-slate-950 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          >
            <h2 className="text-xs font-bold text-blue-500 uppercase tracking-widest">Why Choose Us</h2>
            ...
            {/* Stat Box */}
            <div className="p-6 glass-panel-glow rounded-3xl border border-slate-800 space-y-4">
              <div className="flex items-center space-x-3">
                <div className="bg-blue-500/20 p-2 rounded-xl text-blue-400">
                  <Compass className="h-5 w-5" />
                </div>
                <h4 className="font-bold text-white text-sm">Full ISSB & Test Guidance</h4>
              </div>
          ...
          <div className="lg:col-span-7">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {highlights.map((highlight, index) => (
                <motion.div
                  key={index}
                  className="glass-panel p-6 rounded-2xl border border-slate-900 hover:border-slate-800 transition-colors duration-300"
                ...
                >
                  <div className="bg-slate-950/80 border border-slate-900 w-12 h-12 rounded-xl flex items-center justify-center mb-4">
                    {highlight.icon}
                  </div>
                  <h4 className="font-bold text-white text-lg mb-2">{highlight.title}</h4>
